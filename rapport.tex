...
\begin{rmk}
    Dans l'algorithme (\ref{algo:line-search}), la règle utilisée n'est pas explicitée, mais précisera les conditions "$t$ est trop petit", "$t$ est trop grand", ou "$t$ convient".
\end{rmk}
\begin{algorithm}[ht]
    \centering{
        \fbox{
            \begin{minipage}[t]{150mm}
                ...
                \begin{tabbing}
                    ...
                    \line{LINE-16} \>\> $t=\frac{\alpha+\beta}{2}$\\
                    \>\> ou $t_{\texttt{WP}} = \underset{t}\min$ {\texttt{INTERPOLATION$^3$}$_{\varphi}$}($t$, $\alpha$, $\beta$, $f$, $\nabla f$, $x_k$, $d_k$)\\
...
Dans notre implémentation, on se servira de la formule de mise à jour de $H_{k+1}$. Malgré son grand défaut, elle a l'avantage par rapport aux méthodes recherchés d'être beaucoup plus simple à implémenter...

Il faudra faire particulièrement attention dans les simulations à la positivité de la matrice $H_k$, pour laquelle on peut vérifier la positivité en essayant de la décomposer selon l'algorithme (\ref{algo:cldlt}). Ainsi, on vérifie à la ligne (\ref{BFGS-12}) si la matrice n'est plus définie positive. Si $H_{k+1}$ ne se décompose pas sous la forme $LDL^T$, alors on réinitialise $H_{k+1}$ à $I$. La complexité de l'algorithme (\ref{algo:cldlt}) étant de l'ordre de $O(n^3)$, on tentera d'observer l'impact calculatoire de cette vérification.
