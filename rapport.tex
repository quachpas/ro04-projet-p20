        ...
        \fbox{
            \begin{minipage}[t]{150mm}
                ...
                \begin{tabbing}
                    ...
                    \line{NR-03} \> $\epsilon = 10^{-3}$, si une précision n'est pas précisée.\\
                    {\bf Itérations :}\\
                    \line{NR-04} \> {\bf Tant que} $g(x_k) > \varepsilon$ {\bf ET} $k < N$ {\bf faire :}\\
                    \line{NR-05} \>\> $x_{k+1} = x_k - t_k\nabla g(x_k)^{-1}\cdot g(x_k)$\\
                    \line{NR-06} \>\> $N = N+1$\\
                    \line{NR-07} \>\> $k = k+1$\\
                    %------------------
                \end{tabbing}
...
On fera particulièrement attention à l'inversion de $\nabla g(x_k)$, qui se réalisera à travers une division matricielle gauche. L'opérateur utilisée sera \texttt{\}. Cet opérateur est bien plus efficace que la commande d'inversion \texttt{inv}, qui résout le système d'équations linéaires \cite{doc:scilab-backslash}.
