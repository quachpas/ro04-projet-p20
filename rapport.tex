...
\begin{definition}
    ...
    \begin{align}
        H_{k+1} = H_k & + \left(\frac{(s_ks_k^T + y_k^T H_k y_k)(s_k s_k^T)}{(s_k^T y_k)^2}\right)\nonumber \\
                      & -\left(\frac{s_k y_k^T H_k+ H_k y_k s_k^T}{s_k^T y_k}\right)
        ...
        \fbox{
            \begin{minipage}[t]{150mm}
                ...
                \begin{tabbing}
                    ...
                    \>\> $t_{\texttt{WP}} = \underset{t}\min$ {\texttt{IP$^3$}$_{\varphi}$}($t$, $\alpha$, $\beta$, $f$, $\nabla f$, $x_k$, $d_k$)\\
...
\begin{rmk}
    L'algorithme de recherche linéaire ainsi énoncé ne clarifie pas totalement le calcul de $t$. En détail, si la règle de Wolfe-Powell est utilisée, on tentera de réduire l'intervalle par interpolation cubique jusqu'à retour d'une erreur. Au quel cas, on basculera sur une réduction par dichotomie.
\end{rmk}
...
\begin{rmk}
    Spécifiquement au niveau de l'implémentation, une condition d'arrêt sur l'égalité des deux bornes de l'intervalle est rajoutée. Dans certaines situations, lorsque les paramètres sont spécialement mal choisis, l'intervalle se réduit par dichotomie de façon alternée.
\end{rmk}
...
Pour profiter pleinement de la performance des opérateurs vectoriels de Scilab, on sépare à l'implémentation la somme $\sum_{k=1}^{j-1}l_{jk}d_{kk}$, qui est utilisée à deux reprises à la ligne (\ref{CLDL-05}) et (\ref{CLDL-07}). On se servira d'un vecteur intermédiaire dont les coefficients sont définis par le produit $l_{jk}\cdot d_{kk}$.
        ...
        \fbox{
            \begin{minipage}[t]{150mm}
                ...
                \begin{tabbing}
                    ...
                    \> $N\in \mathbb{N}$ \>:= le nombre d'itérations maximum\\
                    {\bf  Sortie :}\\
                    \> $x_*$ \>:= le minimum de $f$ trouvé, si la méthode\\\>\> converge\\
                    {\bf  Initialisation :}\\
                    \line{NR-01} \> $k = 0$ \>:= le nombre d'itérations actuels\\
                    \line{NR-02} \> $N = 10^3$ \>:= s'il n'est pas précisé\\
                    \line{NR-03} \> $t_0 = 1$ \>:= le pas initial de l'itération $k$, puis \\
                    déterminé par une méthode de recherche linéaire\\
                    \line{NR-04} \> $\varepsilon = 10^{-5}$, si une précision n'est pas précisée.\\
                    {\bf Itérations :}\\
                    \line{NR-05} \> {\bf Tant que} $\lVert \nabla f(x_k) \rVert > \varepsilon$ {\bf ET} $k < N$ {\bf faire :}\\
                    \line{NR-06} \>\> $d_k = -\nabla^2 f(x_k)^{-1}\cdot \nabla f(x_k)$\\ 
                    \line{NR-07} \>\> Choisir $t_k$ selon l'algorithme (\ref{algo:line-search})\\ 
                    \line{NR-08} \>\> $x_{k+1} = x_k + t_k d_k$\\
                    \line{NR-09} \>\> $k = k+1$\\
                    %------------------
                \end{tabbing}
                \normalsize
...
On fera néanmoins attention à l'inversion de $\nabla^2 f(x_k)$, qui se réalisera à travers une division matricielle gauche. L'opérateur utilisée sera \texttt{\textbackslash}. Cet opérateur est bien plus efficace que la commande d'inversion \texttt{inv}, qui résout le système d'équations linéaires \cite{doc:scilab-backslash}.\footnote{Nous sommes par ailleurs limité par les consignes du sujet, qui refuse l'utilisation de la commande \texttt{inv}.}
    ...
    \centering{
        \fbox{
            \begin{minipage}[t]{150mm}
                ...
                \resetline
                \begin{tabbing}
                    aaaA\=aaaaaA\=aaA\=aaA\=aaA\=aaA\kill
                    {\bf  Entrée :} \\
                    ...
                    \line{BFGS-02} \> $N = 10^3$ \>, s'il n'est pas précisé\\
                    \line{BFGS-03} \> $t_0 = 1$ \>:= le pas initial de l'itération $k$, puis \\
                    déterminé par une méthode de recherche linéaire\\
                    \line{BFGS-04} \> $H_0 = I$ \>:= si elle n'est pas définie \\
                    \line{BFGS-05} \> $\varepsilon = 10^{-5}$, si une précision n'est pas précisée.\\
                    {\bf Itérations :}\\
                    \line{BFGS-06} \> {\bf Tant que} $\lVert f(x_k) \rVert> \varepsilon$ {\bf ET} $k < N$ {\bf faire :}\\
                    \line{BFGS-07} \>\> $d_k = - H_{k}\cdot \nabla f(x_k)$\\
                    \line{BFGS-08} \>\> Choisir $t_k$ selon l'algorithme (\ref{algo:line-search})\\
                    \line{BFGS-09} \>\> $x_{k+1} = x_k + t_k d_k$\\
                    \line{BFGS-10} \>\> $s_k = t_k d_k$\\
                    \line{BFGS-11} \>\> $y_k = \nabla f(x_{k+1}) - \nabla f(x_k)$\\
                    \line{BFGS-12} \>\> $H_{k+1} = \left(I - \frac{y_k s_k^T}{s_k^T y_k}\right) H_k \left(I - \frac{s_k y_k^T}{s_k^Ty_k}\right) + \frac{y_k y_k^T}{s_k^T y_k}$\\
                    \line{BFGS-13} \>\> {\bf Si} $H_{k+1}$ ne se décompose pas selon \\
                    \>\>l'algorithme (\ref{algo:cldlt}) {\bf alors} $H_{K+1} = I$\\
                    \line{BFGS-14} \>\> $k = k+1$\\
                    %------------------
                \end{tabbing}
                \normalsize
...
On prête une attention particulière dans les simulations à la positivité de la matrice $H_k$, pour laquelle on vérifie sa positivité en essayant de la décomposer selon l'algorithme (\ref{algo:cldlt}). Ainsi, à la ligne (\ref{BFGS-12}), si la matrice n'est plus définie positive, alors on réinitialise $H_{k+1}$ à $I$. La complexité de l'algorithme (\ref{algo:cldlt}) étant de l'ordre de $O(n^3)$.

\subsubsection{Conditions d'arrêts et précision}
Les conditions d'arrêts sont sensiblement les mêmes que pour l'algorithme (\ref{algo:newton_raphson}).

\subsubsection{Difficultées techniques}
La difficulté technique majeure se situe dans la formule de mise à jour, où il faut prêter un oeil attentif à l'optimalité des opérateurs vectoriels utilisés. Certains facteurs sont scalaires, et on peut préférer la formule développée (\ref{eq:bfgs-hk-developed})
...
\subsubsection{Minimum global}
    ...
    On examine les valeurs propres de la hessienne au point $(1,1)$. Pour montrer que $(1,1)$ est un minimum local, il faut que les deux valeurs propres $\lambda_1$ et $\lambda_2$ soient strictement positives. Cette condition est vérifiée dans l'annexe \ref{ap:calcul-vp-rosenbrock}, donc $(1,1)$ est un minimum local. Comme ce point est le seul point stationnaire de $r$, il est donc son minimum global.

\subsubsection{Application de Newton-Raphson et BFGS}
\begin{figure}[htbp]
    \centering
    \includegraphics[width = 0.45\textwidth]{NR_c1.63.png}
    \caption{Résultats de l'application de la méthode de Newton-Raphson à la recherche du minimum de $r(x,y)_{p=100}$ avec $c = 1.63$}
    \label{fig:rosenbrock-newton-raphson}
\end{figure}

On fait apparaître successivement dans la figure (\ref{fig:rosenbrock-newton-raphson}) les contours et le chemin des itérations successives $x_k$ de la méthode de Newton-Raphson, $\lVert \grad f(x_k) \rVert_2$ en fonction de l'itération $k$, l'ordre de convergence calculée selon $\frac{\lvert \lVert \nabla f(x_{k+1}) \rVert_2 - \lVert \nabla f(x_*) \rVert_2 \rvert}{\lvert \lVert \nabla f(x_{k}) \rVert_2 - \lVert \nabla f(x_*) \rVert_2 \rvert}$ en fonction de l'itération $k$, la moyenne du temps écoulé à chaque itération $k$ pour 5 exécutions de la méthode de Newton-Raphson.

Les résultats affichés sont donnés pour chacune des méthodes de recherche linéaire inexacte qui ont été présentées précédemment. En l'occurence, on a choisi les paramètres de la recherche linéaire par défaut, à l'exception de $c$, le facteur multiplicatif de la ligne (\ref{LINE-10}) de l'algorithme (\ref{algo:line-search}). La raison de ce choix se trouve dans l'ordre de convergence instable de la méthode de Newton-Raphson lors de l'application de la règle de Goldstein. 

Bien qu'elle semble converger plus rapidement avec la règle de Goldstein (plus grands pas\footnote{losanges noirs sur la figure des contours}, moins d'itérations au total), l'instabilité de l'ordre de convergence, et l'irrégularité du temps par itération laisse à penser que la recherche linéaire demande plus de temps. 

Par ailleurs, l'ordre de convergence est momentanément très grand pour les règles d'Armijo, et de Wolfe-Powell, et n'apparaît pas sur la figure. La similitude des résultats entre les règles d'Armijo et de Wolfe-Powell nous laisse penser que l'intervalle de sécurité aura plus tendance à se réduire par la borne supérieure.

La variation du paramètre $c$ influe principalement sur la règle de Golstein, dont les résultats deviennent similaires à ceux des deux autres règles pour une majorité d'autres valeurs.

On constante que l'ordre de convergence global de la méthode de Newton-Raphson n'est pas quadratique, comme on pouvait l'espérer. La forme particulière de la fonction de Rosenbrock est ici la raison d'une vitesse de convergence très faible, à mi-chemin entre superlinéaire et sous-linéaire.

\begin{rmk}
    L'application de la méthode de BFGS à la fonction de Rosenbrock ne donne que des résultats très divergents. Aucun résultat n'a pu être obtenu.
\end{rmk}

\subsection{Reconstitutions d'un signal à partir d'observations}

L'exercice n'a pas été traité par manque de temps.

\subsection{Conformation atomique}
\begin{rmk}
    La partie ici rédigée est incomplète. Le code fourni est également incomplet.
\end{rmk}
Pour résoudre le problème d’optimisation pour N = 4, on définit une fonction \verb conformation \space qui prend en paramètre un vecteur de taille 12 (4 points dans l'espace). On définit $$ x_0=(0,0,0,1,0,0,0,1,0,0,0,1)$$ puis on lance la simulation. On obtient une minimisation de la fonction \verb conformation \space égale à -1.8172164 avec $$ x^*=
\begin{pmatrix}
0 \\
-0.1663029 \\
0 \\
...
Or la valeur théorique construite à partir du minimum de V est de seulement $\frac{-7}{8}$, ou -0.85, soit une différence légèrement inférieure à 1.

Nous avons pu exporter au format CSV les valeurs qui ont été converties en points dans \verb ParaView \space dans \textit{Filtre->Alphabetical->TabletoPoints}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width = 0.30\textwidth]{table.png}
  \caption{Données CSV importées et converties en points}
  \label{fig:table}
\end{figure}

Après quelques modifications de l'affichage nous obtenons la représentation suivante

\begin{figure}[htbp]
  \centering
  \includegraphics[width = 0.45\textwidth]{conformation.eps}
  \caption{Représentation de la conformation atomique}
  \label{fig:conformation}
\end{figure}


C'est une géométrie moléculaire dite tétrahèdrique (polyèdre à 4 faces).
